...
ok('创建目标 201', r.status === 201 && r.json.progress === 30);
const goalId = r.json.id;
r = await api('PUT', `/api/goals/${goalId}`, { progress: 101 });
ok('进度越界被拒 400', r.status === 400);
r = await api('PUT', `/api/goals/${goalId}`, { progress: 80 });
ok('进度更新成功', r.status === 200 && r.json.progress === 80);
...
r = await api('POST', '/api/import', {
  tasks: [{ title: '旧任务', priority: '中', category: '其他' }],
  goals: [{ name: '旧目标', progress: 10 }],
  notes: [{ title: '旧笔记', content: 'x', category: '其他' }],
});
ok('导入成功', r.status === 200 && r.json.imported.tasks === 1 && r.json.imported.goals === 1 && r.json.imported.notes === 1, JSON.stringify(r.json));
r = await api('POST', '/api/import', { tasks: [{ title: '' }] });
ok('无效导入被拒 400', r.status === 400);
r = await api('GET', '/api/tasks');
ok('导入数据可查询', r.status === 200 && r.json.length === 1);
